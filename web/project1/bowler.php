...
<html>
	...
	<body class="project1">
	  ...
	  <div class="jumbotron jumbotron-fluid">
        <div class="container">
            <h1 class="display-4"><?php echo $row['first'].' '.$row['last']; ?></h1>
        </div>
    </div>
    <?php } ?>
    ...
    <table class="container">
      ...
      $queryTotals = $db->prepare("SELECT creation_date, count(*), AVG(total) FROM game WHERE bowler = :bowler group by creation_date;");
      $queryTotals->bindValue(':bowler', $_GET['id'], PDO::PARAM_INT);
      ...
      foreach($queryTotals->fetchAll() as $row) {
        for ($x = 5; $x > $row[count]; $x--) {
          $games[$row[creation_date]] .= '<td></td>';
        }
        $totals[$row[creation_date]] = '<td>'.round($row[avg],2).'</td>';
      }

      foreach($games as $key => $value) {
        echo '<tr><td><b>'.$key.'</b></td>'.$value.$totals[$key].'</tr>'; 
      }; ?>
    </div>
    <br><br>
